...
bool Level::LoadAnimations(cJSON* level)
{
	...
	for (int i = 0; i < cJSON_GetArraySize(animations); i++)
	{
		...
		cJSON* special_anims = cJSON_GetObjectItem(anim_pack, "special_anims");

		int anim_count			= cJSON_GetArraySize(anim_set);
		int special_anim_count  = cJSON_GetArraySize(special_anims);
		int total_anims = (anim_count + special_anim_count);


		FrameSet* frame_data = (FrameSet*)malloc( (total_anims)* sizeof(FrameSet));
		int* Index = (int*)malloc( (total_anims) * sizeof(int));
		...
		for (int k= 0; k < (special_anim_count); k++)
		{
			cJSON* anim = cJSON_GetArrayItem(special_anims, k);
			char* filename = cJSON_GetObjectItem(anim, "filename")->valuestring;
			int frame_count = cJSON_GetObjectItem(anim, "frame_count")->valueint;
			int frame_rate = cJSON_GetObjectItem(anim, "frame_rate")->valueint;
			h = cJSON_GetObjectItem(anim, "custom_h")->valueint;
			w = cJSON_GetObjectItem(anim, "custom_W")->valueint;
			char* id_str = cJSON_GetObjectItem(anim, "id")->valuestring;

			int id = ConvertStringToSymbol(id_str);
			//gCons->ConsPrintf("image source path: %s\n", filename);

			frame_data[anim_count + k] = FrameSet(mRen, filename, frame_count, frame_rate, h, w);

			Index[anim_count + k] = id;

		}

		AnimDataPack* pack = new AnimDataPack(total_anims, frame_data, Index);
...
bool Level::LoadAnimGraph(cJSON* anim_pack)
{
	...
	cJSON* fireables_ids = cJSON_GetObjectItem(anim_pack, "fireables_ids");
	for (int i = 0; i < (int)cJSON_GetArraySize(fireables_ids); i++)
	{
		int id = ConvertStringToSymbol(cJSON_GetArrayItem(fireables_ids, i)->valuestring);
		anim_g->DefineFireables(id);
	}
...
bool Level::LoadHud(cJSON* level)
{
	...
	//Load Fader widget
	char* black = cJSON_GetObjectItem(texs, "fader_black")->valuestring;
	char* red = cJSON_GetObjectItem(texs, "fader_red")->valuestring;
	Fader* f = new Fader(mRen, black, red);
