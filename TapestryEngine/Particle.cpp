...
bool ParticleManager::SpawnParticle(const char* type, const char* name, int x, int y, ParticleTypeData ptdata)
{
	...
	{
		Particle* prt = new P_static(pd->AnimData, pos, int(mParticles.size()), NULL);
		mParticles.push_back(prt);
		mParticles.back()->mFeed.Subscribe(this);
	}
	else if (!strcmp(type, "P_drift"))
	{
		Particle* prt = new P_drift(pd->AnimData, pos, int(mParticles.size()), NULL, ptdata.drift);
		mParticles.push_back(prt);
		mParticles.back()->mFeed.Subscribe(this);
	}
	else
...
//Particle Functions
Particle::Particle(AnimGraph* AnimData, SDL_Rect Pos, int ID, ActorHandleManager* AHM ) : Mobile(AnimData, Pos, Pos, AHM, NULL, NULL, NULL) //Could enable particles to play sounds by replacing second null; worth considering
{
...
//P_drift
P_drift::P_drift(AnimGraph* AnimData, SDL_Rect Pos, int ID, ActorHandleManager* AHM, drift_data data) : Particle(AnimData, Pos, ID, AHM), mData(data)
{
}
